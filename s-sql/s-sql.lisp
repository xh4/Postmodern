...
(defun for-update/share (share-or-update form &rest args)
  (let* ((of-position (position :of args))
         ...
         (skip-locked-position (position :skip-locked args))
         (of-tables (when of-position (subseq args (1+ of-position) no-wait-position))))
    (when (and no-wait-position skip-locked-position)
      (sql-error "Only one of waiting options can be add to the locking clause"))
    `("(" ,@(sql-expand form) ,(format nil " FOR ~:@(~A~)" share-or-update)
          ...
          ,@(when skip-locked-position (list " SKIP LOCKED"))
          ")")))
